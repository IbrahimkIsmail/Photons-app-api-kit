                ...
                <div class="d-flex align-items-center flex-wrap mr-1">
                    ...
                    <div class="d-flex align-items-baseline mr-5">
                        ...
                        <span class="text-dark-50 font-weight-bold" id="kt_subheader_total">
                              <a class="text-dark"
                                 href="{{route('admin.users.clients.index')}}">{{admin('Clients Management')}}</a>
                        </span>
                        </div>
                ...
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="card-title">
                            ...
                            <h3 class="card-label">{{admin('Clients')}}</h3>
                        </div>
...
@section('scripts')
        ...
        var KTDatatablesExtensionButtons = function () {
            ...
            var initTable1 = function () {
                ...
                table.DataTable({
                    ...
                    "ajax": {
                        url: "{{route('admin.users.clients.load')}}",
                        data: function (d) {
                    ...
                    columnDefs: [
                        {
                            ...
                            render: function (data, type, full, meta) {
                                var x = '<a  href="/admin/users/clients/edit/' + full.id + '" type="submit" style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-success btn-elevate btn-pill btn-elevate-air la la-edit fa-lg" id="edit_btn" title="{{admin('Edit')}}"></a>';
                                {{--x = x + '<a type="submit" href="/admintz/categories/details/' + full.id + '"style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-info btn-elevate btn-pill btn-elevate-air la la-eye fa-lg" id="details_btn" title="{{admin('View')}}"></a>';--}}
                                var  x = x + '<button type="submit" style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-danger btn-elevate btn-pill btn-elevate-air la la-window-close fa-lg" id="delete_btn" title="{{admin('Delete')}}"></button>';
                        ...
                        //     targets: -3,
                        //     render: function (data, type, full, meta) {
                        //         return '<img src="' + full.image + '" class="kt-img-rounded" style="width: 50px;height: 50px;" />';
                        //     },
                        // },
                    ],
                });
    ...
    <script>
        ...
        function removeTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admin/users/clients/destroy",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
        ...
        function disableTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admin/users/clients/disable",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
        ...
        function activateTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admin/users/clients/activate",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
