                ...
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="card-title">
                            ...
                            <h3 class="card-label">{{admin('Home Slider')}}</h3>
                        </div>
                        <div class="card-toolbar">
                            ...
                            <!--end::Dropdown-->
                            <!--begin::Button-->
                            <a href="{{route('admin.home_slider.create')}}"
...
@section('scripts')
        ...
        var KTDatatablesExtensionButtons = function () {
            ...
            var initTable1 = function () {
                ...
                table.DataTable({
                    ...
                    "ajax": {
                        url: "{{route('admin.home_slider.load')}}",
                        data: function (d) {
                    ...
                    columnDefs: [
                        {
                            ...
                            render: function (data, type, full, meta) {
                                var x = '<a  href="/admin/home_slider/edit/' + full.id + '" type="submit" style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-success btn-elevate btn-pill btn-elevate-air la la-edit fa-lg" id="edit_btn" title="{{admin('Edit')}}"></a>';
                                {{--x = x + '<a type="submit" href="/admintz/countries/details/' + full.id + '"style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-info btn-elevate btn-pill btn-elevate-air la la-eye fa-lg" id="details_btn" title="{{admin('View')}}"></a>';--}}
                        ...
                        },
                        {
                            targets: -3,
                            render: function (data, type, full, meta) {
                                return '<img src="' + full.image + '" class="rounded-circle" style="width: 50px;height: 50px;" />';
                            },
                        },
                    ],
                });
    ...
    <script>
        ...
        function activateTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admin/home_slider/activate",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
